...
def transform_camera_view_t(
    ...
    R = ru.get_r_matrix(
        [1., 0., 0.], angle=np.deg2rad(camera_elevation_degree))
    XYZ = torch.matmul(XYZ.reshape(-1, 3).float(),
...
def depth_image_to_point_cloud(depth, scale, cx, cy, fx, fy, matrix):
    ...
    position = torch.vstack((X, Y, Z, torch.ones(len(X)).cuda(device))).view(bs, 4, -1)
    position = torch.bmm(matrix.cuda(device).unsqueeze(0).repeat(bs, 1, 1), position)
