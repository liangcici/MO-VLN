...
class ReasoningEnv(Env):
    ...
    def __init__(self, args, rank):
        ...
        eps_data_file = self.episodes_dir + "{split}_{type}.json".format(split=self.split, type=args.reasoning_type)
        with open(eps_data_file, 'r') as f:
            self.eps_data = json.load(f)
        self.eps_data_idx = None
        self.goal_name = None
        self.pred_goal_name = None
    ...
    def load_new_episode(self):
        ...
        self.goal_name = episode['goals']
        self.goal_cat_id = episode['goal_ids']
        self.pred_goal_name = episode['pred_goals']
        self.pred_goal_cat_id = episode['pred_goal_ids']
        if episode['pred_goal_id'] == None:
            self.pred_goal_cat_id = 0
        self.goal_best_position = episode['best_goal']['best_position']
        self.goal_shortest_distance = episode['best_goal']['shortest_distance']
    ...
    def reset(self):
        ...
        self.info['pred_goal_name'] = self.pred_goal_name
        self.info['pred_goal_cat_id'] = self.pred_goal_cat_id
