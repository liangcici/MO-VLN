...
class VLNEnv(Env):
    ...
    def __init__(self, args, rank):
        ...
        if args.with_vln_action:
            eps_data_file = self.episodes_dir + "{split}_with_action_{type}.json".format(
                split=self.split, type=args.reasoning_type)
        else:
            eps_data_file = self.episodes_dir + "{split}_without_action_{type}.json".format(
                split=self.split, type=args.reasoning_type)
    ...
    def load_new_episode(self):
        ...
        self.pred_goal_name = episode['pred_goal']
        # self.pred_goal_cat_id = episode['pred_goal_id']
        if len(episode['pred_goal_id']) == 0 or episode['pred_goal_id'] is None:
            self.pred_goal_cat_id = 0
        else:
            self.pred_goal_cat_id = episode['pred_goal_id'][-1]
