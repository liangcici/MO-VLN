...
from envs.ue5.objectgoal_env import ObjectGoalEnv
from constants import color_palette
import envs.utils.pose as pu
import agents.utils.visualization as vu


class UE5_FBE_Env_Agent(ObjectGoalEnv):
    ...
    def __init__(self, args, rank):
        ...
        if args.sem_seg_model_type == 'glip':
            from agents.utils.glip_prediction import SemanticPredGLIP

            self.sem_pred = SemanticPredGLIP(args, categories)
        elif args.sem_seg_model_type == 'grounding_dino':
            from agents.utils.grounding_dino_prediction import SemanticPredGroundingDINO

            self.sem_pred = SemanticPredGroundingDINO(args, categories)
        elif args.sem_seg_model_type == 'grounded_sam':
            from agents.utils.grounded_sam_prediction import SemanticPredGroundedSAM

            self.sem_pred = SemanticPredGroundedSAM(args, categories)
        elif args.sem_seg_model_type == 'glip_sam':
            from agents.utils.glip_sam_prediction import SemanticPredGLIPSAM

            self.sem_pred = SemanticPredGLIPSAM(args, categories)
    ...
    def reset(self):
        ...
        if args.visualize or args.print_images:
            self.vis_image = vu.init_vis_image(self.goal_name, self.categories)
    ...
    def plan_act_and_preprocess(self, planner_inputs):
        ...
        if planner_inputs["wait"]:
            ...
            return np.zeros(self.obs.shape), 0., False, self.info, planner_inputs['goal']

        action, goal = self._plan(planner_inputs)
        # print(action)
        ...
        if action >= 0:
            ...
            return obs, rew, done, info, goal

        else:
            self.last_action = None
            self.info["sensor_pose"] = [0., 0., 0.]
            return np.zeros(self.obs_shape), 0., False, self.info, goal
    ...
    def _plan(self, planner_inputs):
        ...
        if self.last_action == 1:
            ...
            dist = pu.get_l2_distance(x1, y1, x2, y2)
        ...
        # if self.collision_step > 20:
            ...
            # planner_inputs['new_goal'] = 0
            # goal = self._set_random_goal(map_pred)
            # curr_location = (self.msg.location.X, self.msg.location.Y, self.msg.rotation.Yaw)
            # self.reset_world(location=curr_location)

        stg, stop, goal = self._get_stg(map_pred, start, np.copy(goal),
                                        planning_window, not planner_inputs['found_goal'] and planner_inputs['new_goal'],
                                        planner_inputs['free_pred'])
        # print('stg: ', stg)
    ...
    def _get_stg(self, grid, start, goal, planning_window, new_goal, free_pred):
        """Get short-term goal"""
        ...
        if new_goal:
            goal_loc = self._fbe(grid, traversible, start, free_pred)
            if goal_loc is None:
                goal_map = self._set_random_goal(grid)
                goal = add_boundary(goal_map, value=0)
            else:
                goal_map = np.zeros_like(goal)
        ...
        return (stg_x, stg_y), stop, goal_map
    ...
    def _get_sem_pred(self, rgb, use_seg=True):
        if use_seg:
            ...
            semantic_pred = np.zeros((rgb.shape[0], rgb.shape[1], self.num_sem_categories))
            self.rgb_vis = rgb[:, :, ::-1]
        return semantic_pred

    def _fbe(self, grid, traversible, start, free_pred):
        # fontier: unknown area and free area
        # unknown area: not free and not obstacle
        # find nearest frontier and return the gps
        fbe_map = np.zeros_like(grid)
        fbe_map[free_pred > 0] = 1  # first free
        fbe_map[skimage.morphology.binary_dilation(grid, skimage.morphology.disk(4))] = 3  # then dialte obstacle
        ...
        fbe_cpp[skimage.morphology.binary_dilation(fbe_cp, selem)] = 0  # don't know space is 0 dialate unknown space
        ...
        frontier_locations = np.stack([np.where(frontier_map)[0], np.where(frontier_map)[1]]).T
        if len(np.where(frontier_map)[0]) == 0:
            return None
    ...
    def _set_random_goal(self, grid):
        print('set random goal')
        goal = np.zeros_like(grid)
        goal_index = np.where((grid < 1))
        np.random.seed(100)
    ...
    def _visualize(self, inputs):
        ...
        no_cat_mask = sem_map == (self.num_sem_categories + 4)
